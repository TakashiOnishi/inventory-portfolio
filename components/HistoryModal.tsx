...
export function HistoryModal({ item, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-10 flex items-center justify-center bg-black/40 p-4">
      <div className="flex max-h-[80vh] w-full max-w-lg flex-col rounded-lg bg-white p-5 shadow-xl">
        ...
        <div className="overflow-y-auto">
          {error && <p className="text-sm text-red-600">履歴の取得に失敗しました</p>}
          {!error && rows === null && <p className="text-sm text-slate-500">読み込み中…</p>}
          {rows && rows.length === 0 && (
